...
class ApiRequest:
    ...
    # property装饰器，封装属性访问方式，允许在获取属性值时执行一些逻辑
    @property
    def client(self):
    ...
    def knowledge_programmer_interview_chat(
        self,
        query: str,
        knowledge_base_name: str,
        language: str,
        ...
        '''
        对应api.py/chat/knowledge_programmer_interview_chat接口
        '''
        data = {
            "query": query,
            "knowledge_base_name": knowledge_base_name,
        ...
        response = self.post(
            "/chat/knowledge_programmer_interview_chat",
            json=data,
            stream=True,
        )
        return self._httpx_stream2generator(response, as_json=True)
    ...
    def list_config_models(
        ...
        '''
        获取服务器configs中配置的模型列表
        ，返回形式为{"type": {model_name: config}, ...}。
        '''
        data = {
            "types": types,
        }
